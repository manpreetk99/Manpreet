...
namespace BadgerysCreekHotel.Controllers
{
    ...
    public class ManageBookingsController : Controller
    {
        ...
        public async Task<IActionResult> Manage()
        {
            ...
            {
                var user = await _context.Customer.FindAsync(booking.CustomerEmail);
        ...
        public async Task<IActionResult> Edit(int id, [Bind("ID,RoomID,CustomerEmail,CheckIn,CheckOut")] Booking booking)
        {
            if (id != booking.ID)
            {
                return NotFound();
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    booking.TheRoom = _context.Room.Find(booking.RoomID);
                    booking.Cost = booking.TheRoom.Price * booking.CheckIn.DaysOfStay(booking.CheckOut); //my own extension method.
